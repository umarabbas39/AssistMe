...
class BootCompleteReceiver : BroadcastReceiver() {
    ...
    //It will called whenever device restart and set all the alarms from database
    override fun onReceive(context: Context, intent: Intent) {

        if (intent.action == "android.intent.action.BOOT_COMPLETED"){
//            val file = File(context.getExternalFilesDir(null) , "boot.txt")
//            file.createNewFile()
            //GloalScope.launch is kotlin coroutine because we can't access database on main thread
            GlobalScope.launch {
                //getting instance of database and then lists from it and setting alarmss
